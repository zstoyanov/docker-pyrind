...
if echo "$@" | grep -qE "^(/app/)?pyipad( |$)" && ! echo "$@" | grep -qE "\--externalip(=| )"; then
...
echo "Setting owner on $PYRIN_HOME to $PYRIN_USER"
chown $PYRIN_USER:$PYRIN_USER $PYRIN_HOME
echo "Executing: $@ $externalIpArgs"
exec su-exec $PYRIN_USER "$@" $externalIpArgs
